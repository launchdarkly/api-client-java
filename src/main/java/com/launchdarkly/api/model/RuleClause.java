...
public class RuleClause {
   ...
   * @return the RuleClause instance itself
   */
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!RuleClause.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in RuleClause is not found in the empty JSON string", RuleClause.openapiRequiredFields.toString()));
        }
      }
