  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ReleasePoliciesAccessAllowedReason.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in ReleasePoliciesAccessAllowedReason is not found in the empty JSON string", ReleasePoliciesAccessAllowedReason.openapiRequiredFields.toString()));
        }
      }
