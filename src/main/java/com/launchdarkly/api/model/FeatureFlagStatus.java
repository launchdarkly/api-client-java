...
public class FeatureFlagStatus {
   ...
   * @return the FeatureFlagStatus instance itself
   */
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!FeatureFlagStatus.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in FeatureFlagStatus is not found in the empty JSON string", FeatureFlagStatus.openapiRequiredFields.toString()));
        }
      }
