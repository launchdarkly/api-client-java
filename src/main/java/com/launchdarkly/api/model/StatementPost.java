...
public class StatementPost {
   ...
   * @return the StatementPost instance itself
   */
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!StatementPost.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in StatementPost is not found in the empty JSON string", StatementPost.openapiRequiredFields.toString()));
        }
      }
