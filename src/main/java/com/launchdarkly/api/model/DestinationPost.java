...
public class DestinationPost {
   ...
   * @return the DestinationPost instance itself
   */
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!DestinationPost.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in DestinationPost is not found in the empty JSON string", DestinationPost.openapiRequiredFields.toString()));
        }
      }
