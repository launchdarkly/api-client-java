  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!AiConfigsAccessAllowedReason.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in AiConfigsAccessAllowedReason is not found in the empty JSON string", AiConfigsAccessAllowedReason.openapiRequiredFields.toString()));
        }
      }
