...
public class FeatureFlagStatuses {
   ...
   * @return the FeatureFlagStatuses instance itself
   */
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!FeatureFlagStatuses.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in FeatureFlagStatuses is not found in the empty JSON string", FeatureFlagStatuses.openapiRequiredFields.toString()));
        }
      }
      ...
      if (jsonObj.get("items") != null && !jsonObj.get("items").isJsonNull()) {
        ...
        if (jsonArrayitems != null) {
          ...
          for (int i = 0; i < jsonArrayitems.size(); i++) {
            FlagStatusRep.validateJsonElement(jsonArrayitems.get(i));
          };
        }
      }
  }
